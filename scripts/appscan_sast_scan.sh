...
if ! [ -x "$(command -v appscan.sh)" ]; then
    ...
    curl -k -s "https://cloud.appscan.com/api/v4/Tools/SAClientUtilByType?toolType=linux" > $HOME/SAClientUtil.zip
...
if [ "$scoScan" = 'yes' ]; then
    echo "AppScan Prepare using SCO parameter. Command: appscan.sh prepare -es -sco -nc -acceptssl";
    appscan.sh prepare -es -sco -nc -acceptssl
else
    echo "AppScan Prepare. Command: appscan.sh prepare -es -nc -acceptssl";
...
# Upload IRX file
if [ -f "$irxFile" ]; then    
    irxFileId=$(curl -k -s -X 'POST' "https://$serviceUrl/api/v4/FileUpload" -H 'accept:application/json' -H "Authorization:Bearer $appscanToken" -H 'Content-Type:multipart/form-data' -F "uploadedFile=@$irxFile" | grep -oP '(?<="FileId":\ ")[^"]*');
...
scanStatus=$(curl -k -s -X 'GET' "https://$serviceUrl/api/v4/Scans/Sast/$scanId" -H 'accept:application/json' -H "Authorization:Bearer $appscanToken" | jq -r '.LatestExecution | {Status} | join(" ")');
echo $scanStatus
while true ; do 
    scanStatus=$(curl -k -s -X 'GET' "https://$serviceUrl/api/v4/Scans/Sast/$scanId" -H 'accept:application/json' -H "Authorization:Bearer $appscanToken" | jq -r '.LatestExecution | {Status} | join(" ")');
