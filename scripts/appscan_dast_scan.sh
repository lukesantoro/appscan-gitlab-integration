...
if [ -f "$loginDastConfig" ]; then
    loginDastConfigId=$(curl -k -s -X 'POST' "https://$serviceUrl/api/v4/FileUpload" -H 'accept:application/json' -H "Authorization:Bearer $appscanToken" -H 'Content-Type:multipart/form-data' -F "uploadedFile=@$loginDastConfig" | grep -oP '(?<="FileId":\ ")[^"]*');
...
if [ -f "$manualExplorerDastConfig" ]; then
    manualExplorerDastConfigId=$(curl -k -s -X 'POST' "https://$serviceUrl/api/v4/FileUpload" -H 'accept:application/json' -H "Authorization:Bearer $appscanToken" -H 'Content-Type:multipart/form-data' -F "uploadedFile=@$manualExplorerDastConfig" | grep -oP '(?<="FileId":\ ")[^"]*');
...
scanStatus=$(curl -k -s -X 'GET' "https://$serviceUrl/api/v4/Scans/Dast/$scanId" -H 'accept:application/json' -H "Authorization:Bearer $appscanToken" | jq -r '.LatestExecution | {Status} | join(" ")');
echo $scanStatus
