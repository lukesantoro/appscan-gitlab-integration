...
assetGroupIdExist=$(curl -k -s -X 'GET' "https://$serviceUrl/api/v4/AssetGroups" -H 'accept: application/json' -H "Authorization: Bearer $appscanToken" | grep "$assetGroupId")
...
if [ -z "$appId" ]; then
	appId=$(curl -s -k -X POST --header "Authorization: Bearer $appscanToken" --header 'Accept:application/json' --header 'Content-Type: application/json' -d '{"Name":"'"$appscanAppName"'","AssetGroupId":"'"$assetGroupId"'","UseOnlyAppPresences":false}' "https://$serviceUrl/api/v4/Apps" | grep -oP '(?<="Id": ")[^"]*' | head -n 1);
